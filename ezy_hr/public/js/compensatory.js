...
async function update_compensatory_days(frm) {
    if (frm.doc.work_from_date && frm.doc.work_end_date) {
        ...
        frm.clear_table('custom_compensatory');
        ...
        for (let date = from_date; date <= end_date; date.setDate(date.getDate() + 1)) {
            ...
            if (attendance_data) {
                ...
                let child_row = frm.add_child('custom_compensatory');
                if (child_row) {
                    ...
                    frappe.model.set_value(child_row.doctype, child_row.name, 'working_hours', attendance_data.working_hours);
                } else {
                    frappe.throw(__("Unable to add child row to 'custom_compensatory'"));
                }
            }
        }
