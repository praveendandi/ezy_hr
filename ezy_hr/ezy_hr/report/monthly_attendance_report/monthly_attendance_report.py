...
def execute(filters=None):
        ...
        if employee_id:
            ...
            for dates, leave in leave_details.items():
                if "On Leave" in leave:
                    ...
                    employee_data[employee_id]["status_by_date"][str_leave_val] = leave_type_short_forms.get(leave_type_value, leave_type_value)
    ...
    for employee_id, data_row in employee_data.items():
        ...
        for leave_type, count in data_row["leaves_details"].items():
            row[leave_type_short_forms.get(leave_type, leave_type)] = count
