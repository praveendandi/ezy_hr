...
def get_rows(
   ...
   for employee, details in employee_details.items():
       ...
       attendance_for_employee[0].update(
           {"employee": employee, "employee_name": details.employee_name,"date_of_joining":details.date_of_joining,"department":details.department}
       )
       
       attendance_for_employee[0].update(row)
...
def get_leave_type_for_date(employee: str, year: int, month: int, day: int) -> str:
    """Fetches the leave type for a given employee and date."""
    leave_details = frappe.get_all(
        "Attendance",
        filters={
            "employee": employee,
            "attendance_date": "{}-{}-{}".format(year, month, day),
            "status": "On Leave",
        },
        fields=["leave_type"],
        limit=1
    )
    print(f"Leave details for {employee} on {day}/{month}/{year}: {leave_details}")
    leave_type = leave_details[0].get("leave_type") if leave_details else "L"
    print(f"Leave type after fetching: {leave_type}")
    return leave_type


def get_attendance_status_for_detailed_view(
    employee: str, filters: Filters, employee_attendance: Dict, holidays: Dict
) -> List[Dict]:
    ...
    for shift, status_dict in employee_attendance.items():
        ...
        for day in range(1, total_days + 1):
            ...
            # Fetch holiday status from holidays dictionary
...
def get_leave_type_abbreviation(leave_type: str) -> str:
    """Returns the abbreviation of the given leave type."""
