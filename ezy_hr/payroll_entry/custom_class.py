...
def show_payroll_submission_status(submitted, unsubmitted, payroll_entry,hold_slip):
	if not submitted and not unsubmitted and not hold_slip:
		frappe.msgprint(
			_(
		...
		if hold_slip:
			frappe.msgprint(
				_("Below Salary Slips Hold Salary Will Not Be Process: {}").format(
					", ".join(get_link_to_form("Salary Slip", entry) for entry in hold_slip)
				)
			)
	elif unsubmitted:
		frappe.msgprint(
...
def submit_salary_slips_for_employees(payroll_entry, salary_slips, publish_progress=True):
	try:
		...
		is_hold_or_manual_hold = False
		count = 0

		for entry in salary_slips:
			salary_slip = frappe.get_doc("Salary Slip", entry[0])
			is_hold_or_manual_hold = hold_salary_slip_details(entry[0])
			if salary_slip.net_pay < 0:
				unsubmitted.append(entry[0])
			else:
				try:
					if is_hold_or_manual_hold:
						hold_slip.append(entry[0])
					else:
						salary_slip.submit()
		...
		show_payroll_submission_status(submitted, unsubmitted, payroll_entry,hold_slip)
...
# check hold employee
def hold_salary_slip_details(slip_id):
	salary_slip_doc = frappe.get_doc("Salary Slip", slip_id)
	custom_status,custom_manual_hold = frappe.db.get_value("Payroll Employee Detail",{"parent":salary_slip_doc.payroll_entry,"employee":salary_slip_doc.employee},["custom_status","custom_manual_hold"])
	if custom_status or custom_manual_hold:
		return True
